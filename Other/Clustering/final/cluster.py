...
def findOutlier1A(vector,minVal):
    #for i in vector:
...
def findOutlier1(vector, minVal):
    ...
    if vlen%2 == 1:
        median = vector[vlen/2]
    else:
        median = (vlen/2 - 1, vlen/2)
        m1 = vlen/2 - 1
        m2 = vlen/2
        median = (vector[m1] + vector[m2])/2.0

    #print "Median", median
    distMax = abs(median - vector[-1])
    distMin = abs(median - vector[0])
    if(distMax > distMin and distMax>=minVal):
        return (vector.index(maxNum), maxNum)
    elif (distMax < distMin and distMin>=minVal):
        return (vector.index(minNum), minNum)
...
def removeOutlier2N(vector,minVal):
    outlier=0

    vector = sorted(vector, key = lambda x: x[1])

    oldVec = [i for i in vector]
    vector = [i[0] for i in oldVec]

    dists = [0] * len(vector)
    flags = [1] * len(vector)
    for idx, i in enumerate(vector):
        #print vector, i
        dists[idx] = distance(vector, i)
        #print dists[idx]
        if dists[idx] > minVal:
            flags[idx] = 0

    newVec = [oldVec[i][0] for i in range(len(vector)) if flags[i] == 1]
    return newVec
...
def refineCluster(tid,codeDictB,codeDictUb,height,tb,weHave,methodNum):
    ...
    if methodNum==1:
        ...
        newTid=removeOutlier2N(diversityVector, 2800)
    elif methodNum==3:
...
if __name__=="__main__":
    ...
    for i in f:
        count+=1
        i=i.strip('\n\r\t ').split(',')
        i=[j for j in i[1:] if j.split('_')[0]=='1']
        i=[count]+i
        S.append(i)
