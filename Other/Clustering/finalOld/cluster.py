...
if __name__=="__main__":
    ...
    w=0
    if len(sys.argv) < 8:
        print "\n\n\tUsage: %s <chtFile> <dataFile> <alphaVal> <betaVal> <minSup> <numberOfCluster> <methodNum>\n\n"%(sys.argv[0],)
        sys.exit(0)
    ...
    clusters=[]
    for i in oldClus:
        if getQuality(len(i[1],len(i[0]),beta)>qualityThres:
                clusters.append(i)

    pos=0
    j=0
    u1=getQuality(len(clusters[0][1],len(clusters[0][0]),beta)
    u2=getQuality(len(clusters[1][1],len(clusters[1][0]),beta)
    maxU=u1*1.0/u2
    clusters=sorted(clusters, key = keyFunc(beta))
    for i in clusters[2:]:
        u1=u2
        u2=getQuality(len(i[1],len(i[0]),beta)
        if maxU < (u1*1.0/u2):
            maxU=(u1*1.0/u2)
            pos=j
        j+=1
