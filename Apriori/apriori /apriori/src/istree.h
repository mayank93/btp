...
typedef struct {                /* --- item set tree --- */
  ...
  ITEM     maxht;               /* max. height (size of level array) */
  ISTNODE  **lvls;              /* first node of each level */
  SUPP     rule;                /* minimal support of an assoc. rule */
  SUPP     supp;                /* minimal support of an item set */
  SUPP     smax;                /* maximal support of an item set */
  ...
  ITEM     minsz;               /* minimal size of an item set */
  ITEM     maxsz;               /* maximal size of an item set */
...
extern SUPP      ist_supp    (ISTREE *ist, ITEM item);
extern SUPP      ist_suppx   (ISTREE *ist, ITEM *items, ITEM cnt);

extern void      ist_clear   (ISTREE *ist);
extern void      ist_filter  (ISTREE *ist, ITEM size);
extern void      ist_clomax  (ISTREE *ist, int target);
extern void      ist_setsize (ISTREE *ist, ITEM min, ITEM max,
                              int order);
extern void      ist_seteval (ISTREE *ist, int eval, int agg,
                              double thresh, double minimp, ITEM prune);

extern void      ist_init    (ISTREE *ist);
extern ITEM      ist_set     (ISTREE *ist, ITEM *items, SUPP *supp,
                              double *eval);
extern ITEM      ist_rule    (ISTREE *ist, ITEM *rule,  SUPP *supp,
                              SUPP *body, SUPP *head, double *eval);

extern ptrdiff_t ist_report  (ISTREE *ist, ISREPORT *rep);
